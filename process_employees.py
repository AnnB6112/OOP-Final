...
10002 	 Lim 	 Antonio 	 06/19/1988 	 San Antonio De Padua 2, Block 1 Lot 8 and 2, Dasmarinas, Cavite 	 171-867-411 	 52-2061274-9 	 331735646338 	 683-102-776-000 	 663904995411 	 Regular 	 Chief Operating Officer 	 Garcia, Manuel III 	 60,000 	 1,500 	 2,000 	 1,000 	 30,000 	 357.14 
...
10004 	 Reyes 	 Isabella 	 06/16/1994 	 460 Solanda Street Intramuros 1000, Manila 	 786-868-477 	 40-2511815-0 	 341911411254 	 876-809-437-000 	 416946776041 	 Regular 	 Chief Marketing Officer 	 Garcia, Manuel III 	 60,000 	 1,500 	 2,000 	 1,000 	 30,000 	 357.14 
...
for line in lines:
    # Use 2 or more spaces as delimiter to separate columns
    parts = re.split(r'\s{2,}', line.strip())
    
    # Check if we have enough parts. Some lines might be tricky.
    # The last part "Hourly Rate" might be just one space away from previous if it's tight? 
    # But usually these table copies have tabs or multiple spaces.
    
    if len(parts) >= 19:
        # Address is at index 4
        parts[4] = f'"{parts[4]}"'
        
        # Immediate Supervisor is at index 12
        ...
        # Fallback for tab separation if space splitting failed to produce 19 parts
        parts_tab = line.strip().split('\t')
        parts_tab = [p.strip() for p in parts_tab if p.strip()] # Remove empty strings
